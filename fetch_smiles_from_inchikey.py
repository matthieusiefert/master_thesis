"""
Fetch SMILES from InChIKey2D via PubChem API
=============================================
Reads the enriched node table produced by sirius_to_cytoscape.py,
queries PubChem for canonical SMILES using the InChIKey2D column,
and writes an updated table ready for ChemViz2 in Cytoscape.

PubChem REST endpoint used:
  https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/inchikey/{key}/property/CanonicalSMILES,IsomericSMILES/JSON

Usage:
    python fetch_smiles_from_inchikey.py
...
# Only query rows that have an InChIKey2D but no SMILES yet
...
for key in tqdm(needs_smiles, desc="Querying PubChem"):
    # InChIKey2D is only the first block (connectivity layer, 14 chars)
    # PubChem needs the full InChIKey — we append a wildcard-compatible suffix
    # by querying with the 2D key directly (PubChem accepts partial InChIKeys
    # for substructure-level lookup via the /inchikey/ endpoint)
...
print(f"\nSaved → {OUTPUT_TSV}")
print("""
─── ChemViz2 setup in Cytoscape ─────────────────────────────────────────────
1. Apps → App Manager → install ChemViz2 if not already installed
2. Import node_table_SIRIUS_enriched_smiles.tsv via
   File → Import → Table from File  (key column: shared name / row ID)
3. Apps → Chemviz → Settings → Compound column → smiles
4. Select your alkaloid cluster nodes, then:
   Apps → Chemviz → Paint Structures on Selected Nodes
5. For a larger view of any node:
   Select node → Apps → Chemviz → Show Structures in Dialog
─────────────────────────────────────────────────────────────────────────────
""")
